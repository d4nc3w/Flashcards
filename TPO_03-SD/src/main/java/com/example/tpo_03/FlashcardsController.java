...
public class FlashcardsController {
    ...
    @Autowired
    public FlashcardsController(EntryRepository entryRepository, Profiles profileInterface,
                                FileServ fileServ, SpringDataEntryRepository springDataEntryRepository) {
        ...
        for (Entry entry : entries) {
            if (langWord.equals("eng")) {
                if (entry.getEnglish().equals(deleteWord)) {
                    ...
                    springDataEntryRepository.delete(entry);
                    isDeleted = true;
                    break;
                }
            } else if (langWord.equals("pl")) {
        ...
        for(Entry entry : entries) {
            if (langWord.equals("eng")) {
                if (entry.getEnglish().equals(modifyWord)) {
                    ...
                    springDataEntryRepository.save(new Entry(eng, ger, pol));
    ...
    public void PopulateDatabase() {
        if(springDataEntryRepository.count() == 0) {
            System.out.println("Database is emppty, populating...");
