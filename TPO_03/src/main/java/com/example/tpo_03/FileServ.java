...
public class FileServ implements FileService {
    ...
    @Autowired
    public FileServ(EntryRepository entryRepository, @Value("${pl.edu.pja.tpo03.filename}") String filename) {
    ...
    @Override
    public List<Entry> readEntriesFromFileAddToListOnly() {
        List<Entry> entries = new ArrayList<>();
        try (InputStream inputStream = new ClassPathResource(filename).getInputStream();
             BufferedReader br = new BufferedReader(new InputStreamReader(inputStream, StandardCharsets.UTF_16))) {
            String line;
            while ((line = br.readLine()) != null) {
                ...
                if (parts.length == 3) {
                    Entry entry = new Entry(parts[0], parts[1], parts[2]);
                    entries.add(entry);
                    entryRepository.addEntryToList(entry);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
